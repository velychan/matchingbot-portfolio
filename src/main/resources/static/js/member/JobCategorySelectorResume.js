...
function JobCategorySelector(props) {
    ...
    useEffect(() => {
        ...
        fetch(`/api/occupations/name?id=${selectedId}`)
            .then((res) => res.json())
            .then(({jobGroup, jobType, jobRoleName}) => {
                setMain(jobGroup);
                setSub(jobType);
                setDetail(jobRoleName);

                // HTML input hidden 필드에도 반영
                document.getElementById("filter-job-group")?.setAttribute("value", jobGroup);
                document.getElementById("filter-job-type")?.setAttribute("value", jobType);
                document.getElementById("filter-job-role")?.setAttribute("value", jobRoleName);
                document.getElementById("occupationId")?.setAttribute("value", selectedId);
            })
            .catch((err) => {
                console.error("❌ 초기 직무 로딩 실패:", err);
            });
    }, [selectedId]);
